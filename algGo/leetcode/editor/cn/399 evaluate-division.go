...
func calcEquation(equations [][]string, values []float64, queries [][]string) []float64 {
	...
	for _, query := range queries {
		...
		if graph[start] == nil || graph[target] == nil {
			...
			res = append(res, dfs(graph, start, target, map[string]bool{start: true}, 1))
		}
	}
	return res
}

func dfs(graph map[string]map[string]float64, start, target string, visited map[string]bool, curVal float64) float64 {
	if graph[start] == nil || graph[target] == nil {
		return -1
	}
	...
	for to, val := range graph[start] {
		...
		visited[to] = true
		temp := dfs(graph, to, target, visited, curVal*val)
		visited[to] = false
